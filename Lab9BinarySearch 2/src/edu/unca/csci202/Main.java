...
public class Main {
	...
	private static void productionTests() {
		...
		for(int num_items : new int[]{1000,10000, 25000} ) {
			System.out.println("===== Warehouse size " + num_items + " tests. =====");
	...
	public static void interactiveTest() {
		...
		int type = getNumberFromUser(scan,
				"Search Method: (1) iterative, (2) binary");
	...
	private static void performanceTest(int type, int num_ops, int num_items) {
		...
		Iterator<Widget> i = widgetFarm.createItems(num_items);
		while(i.hasNext()){
	...
	private static void priceTest(int type) {
		...
		Iterator<Widget> itr = widgetBarn.priceRangeSearch(25.0, 75.0);
		while(itr.hasNext()) {

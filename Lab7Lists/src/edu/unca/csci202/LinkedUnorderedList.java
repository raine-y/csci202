...
public class LinkedUnorderedList<T extends Comparable<T>> implements UnorderedListADT<T> {
	...
	@SuppressWarnings("hiding")
	private class LULIterator<T> implements Iterator<T> {
		private Node<T> curr;

		public LULIterator(Node<T> node) {
			curr = node;
		}
	...
	public Iterator<T> interator() {
		return new LULIterator<T>(head);
	}

	@Override
	public String toString() {
		String s = "";
		s += "LinkedUnorderedList[";
	...
	public void addAfter(T element, T target) throws NoSuchElementException {
		if (isEmpty())
			throw new NoSuchElementException("could not find element: " + target);

		Node<T> curr = head;
		while (curr != null && curr.element.compareTo(target) != 0) {
			curr = curr.next;
		}
		if (curr == null)
			throw new NoSuchElementException("could not find element: " + target);
